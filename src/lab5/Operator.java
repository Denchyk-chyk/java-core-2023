...
{
    private static <T> Set<T> operation(Set<T> a, Set<T> b, int startSize, boolean incCondition, int sizeInc, int startIndex, Set<T> inner, Set<T> outer, boolean addCondition) //Метод, що є основою для всіх інших
    {
        int size = startSize; //Кількість елементів у кінцевій множині
        ...
        T[] array = Arrays.copyOf(a.getSet(), size); //Створення кінцевої множини на основі множини A
    ...
    public static <T> Set<T> unionSets(Set<T> a, Set<T> b) //Об'єднання множин
    {
        return operation(a, b, a.getSet().length, false,1, a.getSet().length, a, b, false);
    }

    public static <T> Set<T> intersectionSets(Set<T> a, Set<T> b) //Перетин множин
    {
        return operation(a, b, 0, true,1, 0, a, b, true);
    }

    public static <T> Set<T> differanceSets(Set<T> a, Set<T> b) //Різниця множин
    {
        return operation(a, b, a.getSet().length, true,-1, 0, b, a, false);
    }

    public static <T> Set<T> symmetricDiffSets(Set<T> a, Set<T> b) //Симетрична різниця множин
    {
